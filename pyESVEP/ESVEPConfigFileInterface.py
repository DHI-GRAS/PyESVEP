...
from pyESVEP.PyESVEP import PyESVEP


class ESVEPConfigFileInterface():
    ...
    def __init__(self):
        ...
        temp_params = {'model': 'ESVEP', 'resistance_form': 0, 'G_form': 0}
        temp_model = PyESVEP(temp_params)
    ...
    def run(self, is_image):
        ...
        if self.ready:
            if self.params['model'] == "ESVEP":
                model = PyESVEP(self.params)
            else:
